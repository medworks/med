...
$(document).ready(function(){
    $('a.addgroup').click(function(){
        $('input.addgroup-field').css('display','block');
        $('a.delfield').css('display','block');
    });
    $('a.delfield').click(function(){
        $('input.addgroup-field').css('display','none');
        $('a.delfield').css('display','none');
        $('input.addgroup-field').prev('div.formError').css('display','none')
    });
    $('input.submit').click(function(){
        if($('input.addgroup-field').val()!=""){
            $('select.selectgroup').removeClass('validate[required]');
            $('select.selectgroup').prev('div.formError').css('display','none')
        }
        if($('input.addgroup-field').val()==""){
            $('select.selectgroup').addClass('validate[required]');
        }
        if($('select.selectgroup').val()!=""){
            $('input.addgroup-field').removeClass('validate[required]');
            $('input.addgroup-field').prev('div.formError').css('display','none')
        }
        if ($('select.selectgroup').val()!="" && $('input.addgroup-field').val()!=""){
            alert('لطفا یکی از فیلدهای گروه را تکمیل نمایید.')
        }
    });
});