...
var myhr = 3; // for converting time to ir time
var mymin = 30; // for converting time to ir time
...
Calendar.prototype.create = function (_par) {
	...
	if (this.showsTime) {
		...
		(function(){
			...
			var hrs = cal.date.getUTCHours()+myhr;
			var mins = cal.date.getUTCMinutes()+mymin;
			var t12 = !cal.time24;
			var pm = (hrs > 12);
			if (t12 && pm) hrs -= 12;
			...
			cal.onSetTime = function() {
				var pm, hrs = this.date.getUTCHours()+myhr,
					mins = this.date.getUTCMinutes()+mymin;
				if (t12) {
					pm = (hrs >= 12);
					if (pm) hrs -= 12;
					if (hrs == 0) hrs = 12;
...
Date.prototype.equalsTo = function(date) {
	return (date &&
		...
		(this.getUTCHours()+myhr == date.getUTCHours()+myhr) &&
		(this.getUTCMinutes()+mymin == date.getUTCMinutes()+mymin));
};
...
Date.prototype.print = function (str, dateType, useLangNumbers) {
	...
	var s = {};
	var hr = this.getUTCHours()+myhr;
	var pm = (hr >= 12);
	var ir = (pm) ? (hr - 12) : hr;
	var dy = this.getLocalDayOfYear(dateType);
	if (ir == 0)
		ir = 12;
	var min = this.getUTCMinutes()+mymin;
